...
import { casLogin, getUserMenu } from "@/api/user/auth";
...
const unAuthPath = "/403";

router.beforeEach((to, from, next) => {
	if(to.path === unAuthPath) {
		return next();
	}
	let token = getToken();
	if(to.path === casCallBackPath && !token) {
		...
		if(to.query.ticket) {
			...
			casLogin(to.query.ticket).then(res=>{
